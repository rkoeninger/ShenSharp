...
module Analysis =
    ...
    // TODO: do we need to do populate for simple Value exprs, which then get optimized,
...
//            Sequential (List.map (optimize env) (flattenDo exprs))
        | AppExpr(Sym id, args) when not(Map.containsKey id locals) && not(List.contains id specialSymbols) ->
            GlobalApplication(id, internSymbol id globals.Functions, List.map (optimize env) args)
//        | AppExpr(f, args) ->
